...
<html>
    <%
        ...
        int count_belum = LihatData_Rayon.hitungKwh0_belumApprove(bulantahun, kode);
        int count_sudah = LihatData_Rayon.hitungKwh0_sudahAprrove(bulantahun, kode);
        ...
        if (request.getParameter("cari") != null) {
            ...
            response.sendRedirect("data-kwh-0-cek-approve-per-bulan-rayon.jsp");
        }

        if (request.getParameter("commit") != null) {
            session.setAttribute("id_blth", request.getParameter("commit"));
            response.sendRedirect("approve-kwh-0-rayon.jsp");
        }

        if (request.getParameter("commit1") != null) {
            session.setAttribute("id_blth", request.getParameter("commit1"));
            session.setAttribute("link", "data-kwh-0-cek-approve-per-bulan-rayon.jsp");
                    ...
                    <div class="row">
                        <div class="ten wide column">
                            <h4 class="ui top attached center aligned inverted red block header">
                                DATA PELANGGAN KWH 0 UNITUP ${username} BULAN <%=bulantahun.substring(4)%> TAHUN <%=bulantahun.substring(0, 4)%>
                            </h4>
