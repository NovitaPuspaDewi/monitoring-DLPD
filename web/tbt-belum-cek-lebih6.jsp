...
<html>
    <%
        List<LihatData_TBT> kendaraanList = LihatData_TBT.getDataListTBT_lebih6_belum_cek();
        int count = kendaraanList.size();
        String status = null;

        if (request.getParameter("commit") != null) {
            session.setAttribute("id_blth", request.getParameter("commit"));
            session.setAttribute("link", "tbt-belum-cek-lebih6.jsp");
            response.sendRedirect("tbt-belum-cek.jsp");
        }

        if (request.getParameter("lebih6") != null) {
            response.sendRedirect("tbt-belum-cek-lebih6.jsp");
        }

        if (request.getParameter("kurang6") != null) {
            response.sendRedirect("tbt-belum-cek-kurang6.jsp");
        }

        if (request.getParameter("cari2") != null) {
            String unitup = request.getParameter("unitup");
            session.setAttribute("unitup", unitup);
            response.sendRedirect("data-tbt-belum-cek-cari-unitup.jsp");
        }
    %>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Data Pelanggan TBT lebih 6 bulan Belum Cek</title>
        ...
        <div class="ui one column page grid">
            <div class="column">
                <div class="ui info message">
                    ...
                    <p>
                        <b><%=count%></b> data pelanggan TBT lebih dari 6 bulan yang belum dimonitoring<br><br>
