...
<html>
    <%
        List<LihatData_TBT> kendaraanList = LihatData_TBT.getDataListTBT_sudah_cek_unitup(session.getAttribute("username").toString());
        int count = kendaraanList.size();
        String status = null;

        if (request.getParameter("commit") != null) {
            session.setAttribute("id_blth", request.getParameter("commit"));
            session.setAttribute("link", "data-tbt-sudah-cek-rayon.jsp");
            response.sendRedirect("tbt-sudah-cek-rayon.jsp");
        }

        if (request.getParameter("lebih6") != null) {
            response.sendRedirect("tbt-sudah-cek-lebih6-rayon.jsp");
        }

        if (request.getParameter("kurang6") != null) {
            response.sendRedirect("tbt-sudah-cek-kurang6-rayon.jsp");
        }
        ...
        <div class="ui one column page grid">
            <div class="column">
                <div class="ui info message">
                    ...
                    <p>
                        <b><%=count%></b> data pelanggan TBT yang sudah dimonitoring UNITUP ${username}<br><br>
                    ...
                    <div class="row">
                        <div class="ten wide column">
                            <h4 class="ui top attached center aligned inverted red block header">
                                DATA PELANGGAN TBT SUDAH CEK UNITUP ${username}
                            </h4>
