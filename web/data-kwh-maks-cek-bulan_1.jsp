...
<html>
    <%
        ...
        String status = null;
        List<LihatData> kendaraanList = LihatData.getDataListKwhMaks(bulantahun);
        int count = LihatData.hitungKwhMaks(bulantahun);
        int count_belum = LihatData.hitungKwhmaks_belumcek(bulantahun);
        int count_sudah = LihatData.hitungKwhmaks_sudahcek(bulantahun);
        ...
        if (request.getParameter("cari") != null) {
            ...
            response.sendRedirect("data-kwh-maks-cek-bulan_1.jsp");
        }

        if (request.getParameter("commit") != null) {
            session.setAttribute("id_blth", request.getParameter("commit"));
            session.setAttribute("link", "data-kwh-maks-cek-bulan_1.jsp");
            response.sendRedirect("kwh-maks-belum-cek_1.jsp");
        }

        if (request.getParameter("commit1") != null) {
            session.setAttribute("id_blth", request.getParameter("commit1"));
            session.setAttribute("link", "data-kwh-maks-cek-bulan_1.jsp");
            response.sendRedirect("kwh-maks-sudah-cek_1.jsp");
        }
    %>

    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Cek Monitoring Kwh Maks per-Bulan</title>
        ...
        <div class="ui one column page grid">
            <div class="column">
                <div class="ui info message">
                    ...
                    <p>
                        Terdapat <b><%=count%></b> pelanggan dengan pemakaian listrik melebihi kwh maks.<br>
                        <b><%=count_sudah%></b> sudah dimonitoring dan <b><%=count_belum%></b> belum dimonitoring <br><br>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <% String id = kendaraanList.get(i).getmIdpel();
                                            ...
                                            if (status == null) {
                                                status = "Belum Cek, klik untuk monitoring";%>
                                        <td><center><%=status%><br>
                                    <input class="ui tiny red button" type="submit" value="<%=id_blth%>" name="commit"></center></td>
                                    <% } else {
                                        status = "Sudah Cek, klik untuk lihat detail";%>
                                <td><center><%=status%><br>
                                    <input class="ui tiny blue button" type="submit" value="<%=id_blth%>" name="commit1"></center></td>
                                    <%}%>
                                </tr>                                                                   
                                <% }%>
                                </tbody>
