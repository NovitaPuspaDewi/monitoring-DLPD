...
<html>
    <%
        String bulan_tahun_terakhir = session.getAttribute("blth").toString();
        ...
        if (request.getParameter("cari") != null) {
            ...
            response.sendRedirect("dashboard-cari-blth-maks.jsp");
        }
                ...
                <div class="row">
                    <div class="ten wide column">
                        <h4 class="ui top attached center aligned inverted red block header">
                            <center>REKAP KWH MAKS<br>BULAN <%=month%> TAHUN <%=year%></center>
                        </h4>
                        <table class="ui padded table segment attached" id="filmTable">
                            ...
                            <tbody>
                                <%
                                    ...
                                    List<Unitup> list2 = Model.Unitup.getDataList_Unitup_dpm();
                                ...
                                <tr>
                                    <td><%=i + 1%></td>
                                    <td><%=unitup%></td>
                                    <% jumlah = Dashboard.hitung_kwhmaks(bulan_tahun_terakhir, unitup);%>
                                    <td><%=jumlah%></td>
                                    <% sudahcek = Dashboard.hitungKwhmaks_sudahcek(bulan_tahun_terakhir, unitup);%>
                                    <td><%=sudahcek%></td>
                                    <% belumcek = Dashboard.hitungKwhmaks_belumcek(bulan_tahun_terakhir, unitup);%>
                                    <td><%=belumcek%></td>
                                    <% sudahapp = Dashboard.hitungKwhmaks_sudahapprove(bulan_tahun_terakhir, unitup);%>
                                    <td><%=sudahapp%></td>
                                    <% belumapp = Dashboard.hitungKwhmaks_belumapprove(bulan_tahun_terakhir, unitup);%>
                                    <td><%=belumapp%></td>
                                </tr>
                                <% }%>
                            </tbody>
